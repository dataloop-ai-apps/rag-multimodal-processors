"""
Test suite for DOC/DOCX processing.
...
from test_config import TEST_ITEMS, DOC_CONFIG as CONFIG
...
def test_extractor():
    """Test that DocsExtractor works correctly."""
    print(f"\n{'='*60}")
    ...
    extractor = get_extractor('application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    expected_mime_types = [
        'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        'application/vnd.google-apps.document',
    ]
    assert (
        extractor.mime_type in expected_mime_types
    ), f"❌ DocsExtractor should have correct MIME type, got {extractor.mime_type}"
    assert extractor.name == 'Docs', "❌ DocsExtractor should have correct name"
    print("   ✅ DocsExtractor initialized correctly")

    # Test 2: Has extract method
    print("\n2️⃣ Testing extract method...")
    assert hasattr(extractor, 'extract'), "❌ DocsExtractor should have extract method"
    ...
    registry_extractor = get_extractor('application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    assert registry_extractor.mime_type in expected_mime_types, "❌ Registry should return DOC extractor"
    assert registry_extractor.name == 'Docs', "❌ Registry should return DOC extractor"
...
def test_configuration():
    ...
    default_config = {}
    assert (
        'extract_images' not in default_config or default_config.get('extract_images') == True
    ), "❌ Default should extract images"
    print("   ✅ Default config is correct")

    # Test 2: Extraction config
    print("\n2️⃣ Testing extraction configuration...")
    extraction_config = {'extract_images': True, 'extract_tables': True}
...
def run_doc_processing_integration_test(item_id: str, target_dataset_id: str, config: dict):
    """
    Integration test: Test DOC/DOCX processing with a real Dataloop item.
    ...
    """
    print(f"\n{'='*60}")
    print(f"Integration Test: DOC/DOCX Processing")
    print(f"{'='*60}\n")
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("DOC/DOCX Processing Test Suite")
    print("=" * 60)
    ...
    try:
        chunks = run_doc_processing_integration_test(ITEM_ID, TARGET_DATASET_ID, CONFIG)
        print(f"\n✅ All tests passed!")
        sys.exit(0)
    except Exception as e:
