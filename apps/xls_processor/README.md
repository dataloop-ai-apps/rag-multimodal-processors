...
A modular Dataloop application for processing Excel (XLS/XLSX) files into RAG-ready chunks with support for text, tables, and images.

## 🎯 Features

### Text Extraction
- **Sheet Extraction**: Extracts all text from all sheets in Excel files using `pandas` and `openpyxl`
- **Table Extraction**: Optional extraction of tables as markdown format
- **Image Extraction**: Optional extraction of embedded images from worksheets
...
|-----------|------|---------|-------------------|-------------|
| `extract_tables` | boolean | `true` | `true` or `false` | When enabled, extracts all tables from all sheets and includes them as markdown-formatted text in the chunks. Tables are converted to markdown format with headers and rows. |
| `extract_images` | boolean | `true` | `true` or `false` | When enabled, extracts embedded images from Excel worksheets. Images are included in the extraction metadata but not directly in chunk text. |
...
3. **Connect input** (Excel items from dataset)
...
```
[Excel Dataset] → [Excel to Chunks] → [Embedding Model] → [Vector Database]
```
...
Example:
```
spreadsheet.xlsx → spreadsheet_chunk_0001.txt
                → spreadsheet_chunk_0002.txt
                → spreadsheet_chunk_0003.txt
```
...
The Excel processor uses a type-safe, stateless architecture:

```
XLSProcessor (app.py)
    ├── XLSExtractor (xls_extractor.py) - Excel-specific extraction
...
**Pipeline Flow:**
```python
data = XLSExtractor.extract(data)    # Extract text, images, tables
